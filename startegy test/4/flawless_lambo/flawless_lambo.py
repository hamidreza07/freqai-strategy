...
class flawless_lambo(IStrategy):
    ...
    # hyperopt params
    entry_williams_low = DecimalParameter(-100, -50, default=-58.201,space='buy')
    entry_williams_high = DecimalParameter(-100, -50, default=-92.098,space='buy')
    entry_rsi_low = DecimalParameter(0, 30, default=2.342,space='buy')
    entry_rsi_high = DecimalParameter(30, 60, default=51.22,space='buy')
    entry_volume = DecimalParameter(1000, 100000, default=5000,space='buy')

    exit_rsi = DecimalParameter(60, 100, default=60.631,space='sell')
    exit_williams = DecimalParameter(-40, -10, default=-19.308,space='sell')
    exit_volume = DecimalParameter(1000, 100000, default=1000,space='sell')
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
            (dataframe['volume'] > self.entry_volume.value) &
            (dataframe['OBVSlope'] > 0) &
            (dataframe['williamspercent'] <= self.entry_williams_low.value) &
            (dataframe['williamspercent'] > self.entry_williams_high.value) &
            (dataframe['rsi'] > self.entry_rsi_low.value) &
            (dataframe['rsi'] <= self.entry_rsi_high.value) &
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (dataframe['volume'] > self.exit_volume.value) &
                (dataframe['close'] >= dataframe['bb.upper']) &
                (dataframe['williamspercent'] > self.exit_williams.value) |
