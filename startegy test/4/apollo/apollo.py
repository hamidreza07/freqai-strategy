...
class Apollo11(IStrategy):
    ...
    # Signal 1
    s1_ema_xs = IntParameter(low=1, high=20, default=3, space='buy', optimize=True, load=True)
    s1_ema_sm =  IntParameter(low=1, high=20, default=5, space='buy', optimize=True, load=True)
    s1_ema_md =  IntParameter(low=1, high=20, default=10, space='buy', optimize=True, load=True)
    s1_ema_xl =  IntParameter(low=40, high=80, default=50, space='buy', optimize=True, load=True)
    s1_ema_xxl = IntParameter(low=200, high=300, default=240, space='buy', optimize=True, load=True)

    # Signal 2
    s2_ema_input = IntParameter(low=40, high=80, default=50, space='buy', optimize=True, load=True)
    s2_ema_offset_input = IntParameter(low=-20, high=20, default=-1, space='buy', optimize=True, load=True)

    s2_bb_sma_length = IntParameter(low=35, high=70, default=49, space='buy', optimize=True, load=True)

    s2_bb_std_dev_length = IntParameter(low=50, high=80, default=64, space='buy', optimize=True, load=True)
    s2_bb_lower_offset = IntParameter(low=1, high=20, default=3, space='buy', optimize=True, load=True)

    s2_fib_sma_len = IntParameter(low=35, high=70, default=50, space='buy', optimize=True, load=True)
    s2_fib_atr_len = IntParameter(low=10, high=30, default=14, space='buy', optimize=True, load=True)

    s2_fib_lower_value = DecimalParameter(4, 6, default=4.236, space='buy', optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["s2_ema"] = s2_ema_value - s2_ema_value * self.s2_ema_offset_input.value
